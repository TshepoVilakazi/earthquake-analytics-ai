...
SILVER_FILE = ROOT_DIR / "data" / "silver" / "earthquake_silver.csv"
GOLD_DAILY_FILE = ROOT_DIR / "data" / "gold" / "earthquake_daily_summary.csv"
GOLD_MAGNITUDE_FILE = ROOT_DIR / "data" / "gold" / "earthquake_magnitude_summary.csv"

PREDICTIONS_FILE = ROOT_DIR / "data" / "gold" / "earthquake_predictions.csv"
METRICS_FILE = ROOT_DIR / "data" / "gold" / "earthquake_model_metrics.txt"
FORECAST_FILE = ROOT_DIR / "data" / "gold" / "earthquake_forecast.csv"
FEATURE_IMPORTANCE_FILE = ROOT_DIR / "data" / "gold" / "feature_importance.csv"
...
with tab1:
    ...
    st.line_chart(events_over_time, x="event_date", y="total_events")
    ...
    st.bar_chart(top_places, x="place", y="total_events")
...
with tab3:
    ...
    if "magnitude_band" in filtered_df.columns:
        ...
        st.bar_chart(mag_band, x="magnitude_band", y="total_events")
    else:
    ...
    if "magnitude_band" in filtered_df.columns:
        ...
        st.bar_chart(depth_by_mag, x="magnitude_band", y="depth_km")
...
with tab5:
    ...
    if predictions_df is None:
        ...
        col3.metric("Normal Days", f"{len(predictions_df) - len(high_risk_days):,}")
...
with tab6:
    ...
    if forecast_df is None:
        ...
        st.bar_chart(forecast_df, x="forecast_date", y="forecast_total_events")
...
with tab7:
    ...
    if feature_importance_df is None:
        ...
        st.bar_chart(feature_importance_df, x="feature", y="importance")
    ...
    def render_chart(chart_type: str):
        if chart_type == "forecast_risk":
            ...
            if feature_importance_df is not None:
                ...
                st.warning("Feature importance data not found. Run: python -m src.feature_importance")
            ...
            st.info(
                "Try asking: `Draw forecast risk graph`, `Draw feature importance graph`, "
                "`Draw total events over time`, or `Draw average magnitude graph`."
            )
